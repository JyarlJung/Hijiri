...
public class ObjectPool : Singleton<ObjectPool>
{
    ...
    public bool PushToPool(string itemName, GameObject item, Transform parent = null)
    {
        ...
        pool.PushToPool(item, parent ?? transform);
        return true;
    }

    public GameObject PopFromPool(string itemName, Transform parent = null , bool eneble=true)
    {
        PooledObject pool = GetPoolItem(itemName);
        if (pool == null)
            return null;

        return pool.PopFromPool(parent,eneble);
    }
    ...
    public PooledObject GetPoolItem(string itemName)
    {
        ...
        Debug.LogWarning("There's no matched pool list." + itemName + objectPool.Count);
        return null;
    }
    public int GetItemNumber(string itemName)
    {
        return GetPoolItem(itemName).GetItemNumber();
    }
}